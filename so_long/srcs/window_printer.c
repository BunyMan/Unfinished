...
/*   Updated: 2022/06/16 16:38:24 by jbuny-fe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	img_putter(int i, int j, win_s, spr_s)
{
	while (j < m -> width)
	{
		if (m -> map[i][j] == '1')
			mlx_put_image_to_window(m -> mlx,
				m -> win, s -> edge, j * (s -> img_w), i * (s -> img_h));
		else
			mlx_put_image_to_window(m -> mlx,
				m -> win, s -> pool, j * (s -> img_w), i * (s -> img_h));
		if (m -> map[i][j] == 'C')
			mlx_put_image_to_window(m -> mlx,
				m -> win, s -> hat, j * (s -> img_w), i * (s -> img_h));
		else if (m -> map[i][j] == 'E')
			mlx_put_image_to_window(m -> mlx,
				m -> win, s -> hat, j * (s -> img_w), i * (s -> img_h));
		else if (m -> map[i][j] == 'P')
			mlx_put_image_to_window(m -> mlx,
				m -> win, s -> duck_front, j * (s -> img_w), i * (s -> img_h));
		else if (m -> map[i][j] == 'B')
			mlx_put_image_to_window(m -> mlx,
				m -> win, s -> gator, j * (s -> img_w), i * (s -> img_h));
		j++;
	}
}
